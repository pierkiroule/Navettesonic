...
export const FISH_CONTROL_TUNING={
  ...
  freeSwim:{mouthOffset:42,maxSpeedFactor:0.92,accel:0.055,arrivalRadius:420,stopRadius:18},
};
...
const FREE_SWIM_INTENTION_MIN_MS = 9000;
const FREE_SWIM_INTENTION_MAX_MS = 19000;
const FREE_SWIM_LOOKAHEAD = 0.045;
...
function pickWeighted(items) {
  const total = items.reduce((sum, item) => sum + Math.max(0, item.weight || 0), 0) || 1;
  let roll = Math.random() * total;
  return items.find((item) => {
    roll -= Math.max(0, item.weight || 0);
    return roll <= 0;
  })?.type || items[0]?.type || "arc";
}

function clampDancePoint(point, navRadius) {
  const maxRadius = Math.max(80, navRadius);
  const d = Math.hypot(point.x, point.y);
  if (d <= maxRadius) return point;
  const clamped = clampToCircle(point, maxRadius);
  return { ...point, x: clamped.x, y: clamped.y };
}

function createFreeSwimIntention(state, arenaRadius, now) {
  const fish = state.fish || {};
  const navRadius = Math.max(180, getFishMovementRadius(arenaRadius) - FISH_MEMBRANE_PADDING * 1.18);
  const stars = getReadableStars(state.echostory);
  const hasStars = stars.length > 0;
  const type = pickWeighted([
    { type: "arc", weight: 2.7 },
    { type: "spiral", weight: 2.35 },
    { type: "orbit", weight: 2.15 },
    { type: "drift", weight: 1.25 },
    { type: "approach", weight: hasStars ? 2.2 : 0 },
  ]);
  const duration = FREE_SWIM_INTENTION_MIN_MS + Math.random() * (FREE_SWIM_INTENTION_MAX_MS - FREE_SWIM_INTENTION_MIN_MS);
  ...
  const currentRadius = Math.max(90, Math.min(navRadius * 0.92, Math.hypot(fish.x || 0, fish.y || 0) || navRadius * 0.42));
  const direction = Math.random() < 0.5 ? -1 : 1;
  const phase = Math.random() * Math.PI * 2;
  const centerAngle = Math.random() * Math.PI * 2;
  const centerDrift = type === "orbit" ? navRadius * (0.02 + Math.random() * 0.09) : navRadius * (0.04 + Math.random() * 0.14);
  let focus = null;

  if (type === "approach" && hasStars) {
    const unread = stars.filter((star) => !star.previewPlayed && !star.previewPlaying);
    const source = unread.length ? unread : stars;
    focus = source[Math.floor(Math.random() * source.length)] || null;
  }

  const start = { x: Number.isFinite(fish.x) ? fish.x : 0, y: Number.isFinite(fish.y) ? fish.y : 0 };
  const driftDistance = navRadius * (0.38 + Math.random() * 0.42);
  const driftCurve = navRadius * (0.12 + Math.random() * 0.28) * direction;
  ...
  return {
    ...
    startX: start.x,
    startY: start.y,
    startAngle: type === "drift" ? heading : radialAngle,
    baseRadius: currentRadius,
    targetRadius: Math.max(110, Math.min(navRadius * 0.9, currentRadius * (type === "spiral" ? (Math.random() < 0.5 ? 0.42 : 1.28) : 0.8 + Math.random() * 0.32))),
    radiusAmp: navRadius * (0.025 + Math.random() * 0.055),
    turns: type === "orbit" ? 0.78 + Math.random() * 0.72 : type === "spiral" ? 0.72 + Math.random() * 0.82 : 0.22 + Math.random() * 0.34,
    arcSpan: (0.65 + Math.random() * 0.75) * Math.PI,
    centerX: Math.cos(centerAngle) * centerDrift,
    centerY: Math.sin(centerAngle) * centerDrift,
    focusX: focus ? focus.x : 0,
    focusY: focus ? focus.y : 0,
...
function evaluateFreeSwimIntention(intention, progress) {
  const t = clamp01(progress);
  const e = smoothstep(t);
  const breath = Math.sin(t * Math.PI * 2 + (intention.phase || 0));
  let x = intention.x || 0;
  let y = intention.y || 0;

  if (intention.type === "orbit") {
    const angle = (intention.startAngle || 0) + intention.direction * intention.turns * Math.PI * 2 * t;
    const radius = intention.baseRadius + intention.radiusAmp * breath;
    x = (intention.centerX || 0) + Math.cos(angle) * radius;
    y = (intention.centerY || 0) + Math.sin(angle) * radius;
  } else if (intention.type === "spiral") {
    const angle = (intention.startAngle || 0) + intention.direction * intention.turns * Math.PI * 2 * t;
    const radius = lerpNumber(intention.baseRadius, intention.targetRadius, e) + intention.radiusAmp * breath;
    x = Math.cos(angle) * radius;
    y = Math.sin(angle) * radius;
  } else if (intention.type === "approach") {
    const cx = Number.isFinite(intention.focusX) ? intention.focusX : 0;
    const cy = Number.isFinite(intention.focusY) ? intention.focusY : 0;
    const startAngle = Math.atan2((intention.startY || 0) - cy, (intention.startX || 0) - cx);
    const startRadius = Math.max(120, Math.hypot((intention.startX || 0) - cx, (intention.startY || 0) - cy));
    const endRadius = 86 + Math.abs(Math.sin(intention.phase || 0)) * 86;
    const angle = startAngle + intention.direction * (0.48 + intention.turns * 0.42) * Math.PI * 2 * t;
    const radius = lerpNumber(startRadius, endRadius, e) + intention.radiusAmp * breath * 0.62;
    x = cx + Math.cos(angle) * radius;
    y = cy + Math.sin(angle) * radius;
  } else if (intention.type === "drift") {
    const heading = intention.startAngle || 0;
    const side = heading + Math.PI / 2;
    const forward = Math.sin(e * Math.PI * 0.5) * intention.driftDistance;
    const curve = Math.sin(e * Math.PI) * intention.driftCurve;
    x = (intention.startX || 0) + Math.cos(heading) * forward + Math.cos(side) * curve;
    y = (intention.startY || 0) + Math.sin(heading) * forward + Math.sin(side) * curve;
  } else {
    const angle = (intention.startAngle || 0) + intention.direction * intention.arcSpan * e;
    const radius = lerpNumber(intention.baseRadius, intention.targetRadius, e) + intention.radiusAmp * breath;
    x = (intention.centerX || 0) + Math.cos(angle) * radius;
    y = (intention.centerY || 0) + Math.sin(angle) * radius;
  }

  return clampDancePoint({ x, y }, intention.navRadius || 600);
}

function resolveFreeSwimIntention(state, arenaRadius, now) {
  const current = state.fish?.freeSwimTarget || null;
  const expired = !current || current.kind !== "dance" || now - (current.bornAt || 0) >= (current.duration || FREE_SWIM_INTENTION_MIN_MS);
  const intention = expired ? createFreeSwimIntention(state, arenaRadius, now) : current;
  const progress = (now - intention.bornAt) / Math.max(1, intention.duration || FREE_SWIM_INTENTION_MIN_MS);
  const lookahead = Math.min(1, progress + FREE_SWIM_LOOKAHEAD);
  const point = evaluateFreeSwimIntention(intention, lookahead);
  const breathingSpeed = 0.58 + Math.sin(clamp01(progress) * Math.PI) * 0.28 + Math.sin(clamp01(progress) * Math.PI * 2 + (intention.phase || 0)) * 0.08;
  const motifSpeed = intention.type === "drift" ? 0.72 : intention.type === "orbit" ? 0.9 : intention.type === "spiral" ? 0.84 : 0.82;

  return {
    ...intention,
    ...point,
    progress: clamp01(progress),
    speedScale: Math.max(0.42, Math.min(1.04, breathingSpeed * motifSpeed)),
  };
}
...
export function tickFishEngine(state,{swimSpeed=1,arenaRadius=DEFAULT_ARENA_RADIUS}={}) { /* trimmed by keeping exact logic moved */
  ...
  if (arenaBlob) {
    ...
    const speed=Math.hypot(limitedVx,limitedVy),moveAngle=speed>0.035?Math.atan2(limitedVy,limitedVx):currentAngle,angleEase=shouldFreeSwim?0.024+Math.min(0.026,speed*0.0035):0.055+Math.min(0.055,speed*0.006),angle=speed>0.035?lerpAngle(currentAngle,moveAngle,angleEase):currentAngle;
