...
export function createSlalomCircuitFromBubbles(bubbles = []) {
  try {
    const usable = Array.isArray(bubbles)
      ? bubbles.filter((bubble) =>
          Number.isFinite(bubble.x) && Number.isFinite(bubble.y)
        )
      : [];
    ...
    const sorted = [...usable].sort((a, b) => {
      const aa = Math.atan2(a.y || 0, a.x || 0);
      const bb = Math.atan2(b.y || 0, b.x || 0);
      return aa - bb;
    });

    const beacons = sorted.map((bubble, index) => {
      const angle = Math.atan2(bubble.y || 0, bubble.x || 0);
      const distance = Math.max(
        180,
        Math.hypot(bubble.x || 0, bubble.y || 0)
      );

      const side = index % 2 === 0 ? -1 : 1;
      const tangent = angle + Math.PI / 2;
      const slalomOffset = 72 + (bubble.r || 70) * 0.32;
      const radialBreath = index % 2 === 0 ? 0.88 : 1.12;

      return {
        id: `auto-beacon-${bubble.id || index}`,
        x:
          Math.cos(angle) * distance * radialBreath +
          Math.cos(tangent) * slalomOffset * side,
        y:
          Math.sin(angle) * distance * radialBreath +
          Math.sin(tangent) * slalomOffset * side,
    ...
    return beacons.length >= 3 ? beacons : createDefaultTraceCircuit();
  } catch (error) {
    console.warn("[Soon] fallback circuit slalom", error);
    return createDefaultTraceCircuit();
  }
}
