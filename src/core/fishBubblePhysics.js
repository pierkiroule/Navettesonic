...
export function pushBubblesFromFish(bubbles = [], fish = {}, fishDepth = 1) {
  ...
  return bubbles.map((bubble) => {
    ...
    const nextVY = (previousVY + pushY) * 0.985;
    const impulseMagnitude = Math.hypot(nextVX, nextVY);
    if (impulseMagnitude < 0.0015 && pushX === 0 && pushY === 0) {
    ...
    if (dist >= stickThreshold && dist > 0) {
      ...
      return { ...bubble, x: Math.cos(angle) * boundaryRadius, y: Math.sin(angle) * boundaryRadius, pushVX: nextVX * 0.93, pushVY: nextVY * 0.93 };
    }
