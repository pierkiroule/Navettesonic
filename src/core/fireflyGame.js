...
const TRIANGLE_HEAD_PUSH_RADIUS = 92;
const TRIANGLE_HEAD_PUSH_FORCE = 0.68;
const TRIANGLE_FRICTION = 0.968;
const TRIANGLE_ROTATION_FRICTION = 0.972;
const TRIANGLE_ROTATION_PUSH_FACTOR = 0.0032;
...
function tryCollectFirefly(fish, firefly, now) {
  ...
  if ((firefly.touchStage || 0) === 0) {
    firefly.touchStage = 1;
    pushFireflyFromHead(firefly, fish);
    void triggerFireflyPreview(firefly, now);
    return false;
  }

  if ((firefly.touchStage || 0) >= 1 && firefly.hasPlayedPreview) {
