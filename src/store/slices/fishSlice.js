...
if(type==="inspi"){const extraInwardPush=Math.max(90,basePush*2.6);nx-=Math.cos(fishAngle)*extraInwardPush;ny-=Math.sin(fishAngle)*extraInwardPush;const pushed=clampToCircle({x:nx,y:ny},maxDistance);nx=pushed.x;ny=pushed.y;const spawnAngle=Number.isFinite(angle)?angle:Math.atan2(s.fish?.y||0,s.fish?.x||1);const detachFromMembrane=Math.max(170,(localRadius||DEFAULT_ARENA_RADIUS)*0.14);const spawnRadius=Math.max(90,localRadius-detachFromMembrane);const spawn=clampToCircle({x:Math.cos(spawnAngle)*spawnRadius,y:Math.sin(spawnAngle)*spawnRadius},DEFAULT_ARENA_RADIUS*1.45);const picked=randomBucketSample();nextBubbles=[...nextBubbles,{id:makeId("bubble"),label:picked?.name||"Bulle sonore",x:spawn.x,y:spawn.y,r:70+Math.random()*16,hue:Math.floor(160+Math.random()*170),depth:clampDepth(s.fish?.depth||1),sampleId:picked?.id||"tone-water"}];}
