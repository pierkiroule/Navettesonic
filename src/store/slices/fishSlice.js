...
const nextEchostory=s.echostory?{...s.echostory,stars:stars.map((star)=>{
  ...
  if(type==="expi"){
    const attachedStars=stars.filter((item)=>item?.attachedToContour&&item.id!==star.id);
    const baseAngle=Math.atan2(s.fish?.y||0,s.fish?.x||0);
    const angleStep=0.18;
    const starAngle=baseAngle+(attachedStars.length*angleStep);
