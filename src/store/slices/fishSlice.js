...
if(type==="inspi"){const spawnAngle=Number.isFinite(angle)?angle:Math.atan2(s.fish?.y||0,s.fish?.x||1);const spawnRadius=Math.max(120,localRadius-140);const spawn=clampToCircle({x:Math.cos(spawnAngle)*spawnRadius,y:Math.sin(spawnAngle)*spawnRadius},DEFAULT_ARENA_RADIUS*1.6);const picked=randomBucketSample();nextBubbles=[...nextBubbles,{id:makeId("bubble"),label:picked?.name||"Bulle sonore",x:spawn.x,y:spawn.y,r:70+Math.random()*16,hue:Math.floor(160+Math.random()*170),depth:clampDepth(s.fish?.depth||1),sampleId:picked?.id||"tone-water"}];}
