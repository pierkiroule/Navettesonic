...
if(type==="inspi"){const centerRepulse=Math.max(220,basePush*4.8);nx-=Math.cos(fishAngle)*centerRepulse;ny-=Math.sin(fishAngle)*centerRepulse;const pushed=clampToCircle({x:nx,y:ny},Math.max(40,maxDistance-200));nx=pushed.x;ny=pushed.y;const spawnAngle=Number.isFinite(angle)?angle:Math.atan2(s.fish?.y||0,s.fish?.x||1);const detachFromMembrane=Math.max(340,(localRadius||DEFAULT_ARENA_RADIUS)*0.34);const spawnRadius=Math.max(65,localRadius-detachFromMembrane);const spawn=clampToCircle({x:Math.cos(spawnAngle)*spawnRadius,y:Math.sin(spawnAngle)*spawnRadius},Math.max(95,DEFAULT_ARENA_RADIUS*0.68));const picked=randomBucketSample();nextBubbles=[...nextBubbles,{id:makeId("bubble"),label:picked?.name||"Bulle sonore",x:spawn.x,y:spawn.y,r:70+Math.random()*16,hue:Math.floor(160+Math.random()*170),depth:clampDepth(s.fish?.depth||1),sampleId:picked?.id||"tone-water"}];}
