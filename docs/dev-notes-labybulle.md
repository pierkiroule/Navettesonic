...
### Note CI (refonte arènes)
- Vérifications fiables pour cette série de commits: `node --check src/core/roseFishSchool.js` puis `npm run build`.
- `npm test` reste partiellement en échec dans ce repo à cause de modules/fichiers manquants sous `src/features/arena/*` (hors scope de cette refonte).
