...
function createHarness({ mode = 'compo' } = {}) {
  ...
  const stateRef = { current: { interactionMode: 'swim', mode, fish: { depth: 1 }, viewZoom: 0, circuitAutopilot: false, bubbles: [] } };
...
test('double tap en compo ouvre le menu contextuel circulaire', () => {
...
test('double tap hors compo conserve le changement de profondeur', async () => {
  const { api, calls } = createHarness({ mode: 'reso' });
  api.handlePointerDown(event(1, 500, 500));
  api.handlePointerUp(event(1, 500, 500));
